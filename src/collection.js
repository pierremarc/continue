...
define(['backbone', 'underscore', 'jquery', 'eproxy', 'config', 'logger'],
function(Backbone, _, $, P, config, logger)
{
    'use strict';
        
    var mainApiUrl = config.apiUrl;
    ...
    var Collection = Backbone.Collection.extend({
        ...
        _generateModel: function(){
            this.model = Backbone.Model.extend({
                urlRoot: mainApiUrl + this.modelName + '/',
                idAttribute: 'id',
                synchronize: function (model, mapping){
                    var attrs = model;
                    if('toJSON' in model 
                    && _.isFunction(model.toJSON)){
                        attrs = model.toJSON();
                    }
        ...
        sync: function(method, model, options){
            ...
            if('query' in madeenOptions){
                ...
                if('read' === method){
                    ...
                    if(isListRequest){
                        backboneOptions.url = makePath(mainApiUrl, 
        ...
        /**
        * Used to store changes on the API
        */
        graphCreate: function(subject, predicate, obj, callback, ctx){
            var options = {};
            options.url = makePath(_.result(subject, 'url'), predicate, this.modelName);
            
            var collection = this;
            options.success = function () {
                collection.clearFragments();
                if (callback) callback.apply(ctx, arguments);
            };
        ...
        queryPage: function(name, query, page, callback, ctx){
            ...
            if (name in this.fragments 
                ...
                var params = _.extend({page:page}, _.result(query, 'params'));
                if('command' in query){
